...
CoWrite 实现了一套完整的混合搜索系统，结合学术论文搜索（Google Scholar）和实时网页搜索（TheNews + Smart Search），为用户提供全面、准确的研究素材。
...
- **Google Scholar** = 学术共识 / 理论基础
  - 用于搜索权威学术论文
  - 提供研究共识和理论支撑
  - 通过 SerpApi 访问，需要 INTEGRATIONS_API_KEY

- **TheNews** = 新闻资讯 / 行业动态
  - 用于搜索实时新闻和行业资讯
  - 提供产业实践和最新趋势
  - 需要配置 THENEWS_API_TOKEN

- **Smart Search** = 网页搜索 / 综合信息
  - 用于搜索网页内容和综合信息
  - 提供更广泛的实时内容
  - 需要 INTEGRATIONS_API_KEY
...
- **状态**：已启用
- **API 密钥**：需要 `INTEGRATIONS_API_KEY`
- **说明**：通过 SerpApi 访问 Google Scholar，提供学术论文搜索

### TheNews

- **状态**：需配置
- **API 密钥**：需要配置 `THENEWS_API_TOKEN`
- **获取方式**：访问 TheNews 官网注册账号获取 API Token
- **说明**：提供实时新闻和行业资讯搜索
...
2. **P2**：转写学术关键词
3. **P3**：转写网页查询
4. **并行搜索**：Google Scholar + TheNews + Smart Search
5. **P4**：对齐搜索结果
6. **P5**：生成结构化摘要
7. **保存**：存入知识库
8. **展示**：显示搜索分析
