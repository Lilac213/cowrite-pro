...
### 核心改进（2025-02-06）
我们已经完全重写了 Research Retrieval Agent 和 Research Synthesis Agent，使用新的 `---THOUGHT---` / `---JSON---` 分离模式，确保 100% 的 JSON 解析成功率。

**关键变化**：
1. ✅ LLM 可以在 `---THOUGHT---` 中自由表达，不受 JSON 格式约束
2. ✅ 系统只解析 `---JSON---` 部分，避免解析思考内容
3. ✅ 集成三个外部 API（Google Scholar、TheNews、Smart Search）
4. ✅ 详细的日志记录和错误处理
5. ✅ 并行搜索，提高效率

详细说明请参考：[100% 搜索成功率实现方案](./100_PERCENT_SUCCESS_IMPLEMENTATION.md)
...
**原因**：LLM 没有按照要求的格式输出 `---JSON---` 标记

**解决方案**：
1. 查看返回的 `raw_content` 字段，检查 LLM 的原始输出
2. 查看 Edge Function 日志获取详细信息
3. 如果 LLM 持续不遵守格式，可能需要：
...
**原因**：LLM 输出中缺少 `---JSON---` 标记

**调试步骤**：
1. 检查 Edge Function 日志中的 "DeepSeek 返回内容"
2. 确认 LLM 是否收到了正确的 system prompt
3. 验证 DeepSeek API 密钥是否有效
...
**原因**：外部 API 调用失败（不影响整体流程，只是减少结果数量）

**解决方案**：
1. 检查 `INTEGRATIONS_API_KEY` 是否正确配置
2. 查看具体的 API 错误信息（通过 Edge Function 日志）
3. 确认 API 端点是否可访问
4. 注意：单个 API 失败不会导致整个搜索失败

---

## 旧版错误处理（已弃用，保留用于参考）

以下内容是之前版本的错误处理逻辑，新版本已经不再需要这些修复。
...
1. 移除 JSON 注释（`//` 和 `/* */`）
2. 修复控制字符（转义未转义的 `\n`, `\r`, `\t`, `\b`, `\f`）
3. 移除尾随逗号（如 `{"key": "value",}` → `{"key": "value"}`）
4. 修复未加引号的属性名（如 `{name: "value"}` → `{"name": "value"}`）
5. 修复缺少逗号的情况：
   - 字符串值后直接跟属性名：`"value" "key":` → `"value", "key":`
   - 数字/布尔值后直接跟属性名：`123 "key":` → `123, "key":`
   - 对象/数组结束后直接跟属性名：`{} "key":` → `{}, "key":`
6. 移除多余的连续逗号（如 `{"a":"b",,,"c":"d"}` → `{"a":"b","c":"d"}`）
