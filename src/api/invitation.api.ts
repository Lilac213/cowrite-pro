...
export async function createInvitationCode(credits: number): Promise<InvitationCode> {
  ...
  const { data, error } = await supabase
    ...
    .insert({ code, credits, created_by: user.id } as any)
    .select()
    .single();
...
export async function useInvitationCode(code: string, userId: string): Promise<void> {
  ...
  const { error: updateError } = await supabase
    .from('profiles')
    .update({ available_credits: profile.available_credits + inviteCode.credits } as any)
    .eq('id', userId);
  if (updateError) throw updateError;

  const { error: incrementError } = await supabase
    .from('invitation_codes')
    .update({ used_count: inviteCode.used_count + 1 } as any)
    .eq('id', inviteCode.id);
