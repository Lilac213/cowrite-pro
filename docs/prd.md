...
### 2.1 整体流程图
创建项目 → 明确需求 → 资料查询 → 文章结构 → 段落结构 → 个人素材 → 文章生成 → 内容审校 → 终版输出
...
## 7. 状态机
init → confirm_brief → knowledge_selected → article_structure_confirmed → paragraph_structure_completed → drafting → content_review → completed
