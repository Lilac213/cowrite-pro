...
**P4：OpenAlex + Tavily + 个人素材库 + 参考文章库搜索**
...
- 剔除明显无关或噪声内容
- 不合并写作，仅做筛选与分类
- 输出结构包含：学术核心内容、网页补充内容、个人素材库内容、参考文章库内容、丢弃内容

**P6：英文资料提炼与翻译**
- 对搜索出的英文资料（学术论文、网页内容）进行数据和观点提炼
- 将提炼出的数据和观点翻译成中文展示
- 保留原始英文来源链接
- **翻译和归纳总结在 Top 10 条结果筛选后进行**
...
**P8：生成综合摘要**
- 点击【生成综合摘要】时，系统需考虑明确需求阶段生成的需求文档内容
- 罗列出与需求文档相关的数据和观点
- 对需求文档进行补充和论证
- 综合成一份后续生成文章结构时能直接引用的版本
- 此步骤完成后，用户可点击进入下一步（文章结构）

**资料查询页面展示**
- 搜索结果以卡片形式展示，包含来源、发布时间、摘要
- 明确标识内容来源：OpenAlex、Tavily、个人素材库、参考文章库
- 用户可勾选认可的信息
...
**OpenAlex + Tavily + 个人素材库 + 参考文章库搜索**
- 使用英文关键词在 OpenAlex 搜索学术论文
- 使用英文查询语句在 Tavily 搜索实时内容
- 同步查询个人素材库，匹配相关关键词
- 同步查询参考文章库，匹配相关关键词
- 搜索结果明确标识来源：OpenAlex、Tavily、个人素材库、参考文章库
- **搜索结果优先展示关联性+实时性最高的 Top 10 条结果**
...
- 明确区分学术共识与行业实践/现实应用
- 避免编造结论，不确定的地方需标注
- 输出结构包含：背景总结、学术洞察、行业洞察、个人素材库相关内容、参考文章库相关内容、开放性问题或争议、建议的写作角度
...
**知识库管理**
- 分来源卡片展示（OpenAlex、Tavily、个人素材库、参考文章库）
- 信息勾选与保存
- 知识库长期存储
- 整理观点、论据或案例
- 资料查询增加按钮一键收藏到参考文章库
...
请分析以下【需求文档内容】和【中文需求】，判断其中涉及的研究意图，
...
个人素材库结果：
{{personal_notes_results}}

参考文章库结果：
{{reference_library_results}}

输出格式（JSON）：
{
  \"academic_core\": [],
  \"web_supporting\": [],
  \"personal_notes_supporting\": [],
  \"reference_library_supporting\": [],
  \"discarded\": []
}
```
...
请对以下【英文资料】进行数据和观点提炼，并将提炼出的内容翻译成中文。
...

{
  ...
  \"personal_notes_insights\": [
    {
      \"point\": \"\\",
      \"evidence_source\": \"personal_notes\"
    }
  ],
  \\"reference_library_insights\\": [
    {
      \"point\": \"\",
      \"evidence_source\\": \"reference_library\"
    }
  ],
...
#### P7：生成综合摘要 Prompt
```
你是 CoWrite 的综合摘要生成模块。

请基于以下输入，生成一份综合摘要，用于后续文章结构生成时直接引用。
...
- 资料查询中整理观点、论据或案例
- **搜索结果优先展示关联性+实时性最高的 Top 10 条结果，之后再进行翻译和归纳总结**

### 6.5 提示词使用规则
- 文章结构生成：使用文章结构生成 Prompt，**必须参考资料查询时生成的可引用版本综合摘要中的数据和观点**
...
- 同步查询 OpenAlex、Tavily、个人素材库、参考文章库
- 搜索结果以卡片形式展示，明确标识来源
- **搜索结果优先展示关联性+实时性最高的 Top 10 条结果**
- **英文资料需提炼数据和观点并翻译成中文展示，翻译和归纳总结在 Top 10 条结果筛选后进行**
- 用户可勾选认可的信息纳入本文参考资料
- 若用户需要额外查询补充内容，可在搜索框中输入关键词进行补充搜索
- 点击【生成综合摘要】时，需考虑需求文档内容，罗列出与其相关的数据和观点，对需求文档进行补充和论证，并综合成一份后续生成文章结构时能直接引用的版本
