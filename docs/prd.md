...
│  │   └─ DraftContent（草稿正文）
│  │       ├─ 流式输出草稿内容
│  │       ├─ CitationMarker（引用标记 - 数字标识[1][2]等）
│  │       │   └─ 点击显示资料信息弹窗（资料标题、摘要、URL等）
│  │       ├─ ParagraphEditableArea（段落可编辑区域）
│  │       │   ├─ 用户可随时点击段落进行编辑
│  │       │   ├─ 编辑时段落进入编辑模式，显示编辑框
│  │       │   ├─ 编辑完成后失焦或点击保存，触发段落更新
│  │       │   └─ 段落更新后，自动调用 LLM 分析该段落并给出建议
│  │       └─ ParagraphClickable（段落可点击区域）
│  │           └─ 点击段落后，右侧显示该段落的分析与建议
│  └─ RightPanel（右侧AI互动与段落逻辑建议区，宽度占比较小）
...
1. **左侧主编辑区（宽度占比更大）**
   ...
   - **草稿正文**：
     ...
     - **引用标记采用数字标识**：在引用资料搜索内容的地方用数字标识标记，如[1]、[2]等
     - 点击数字标识后弹窗显示资料信息，包括：
       - 资料标题
       - 资料摘要
       - 资料来源 URL
     - **段落可编辑区域**：
       - 用户可随时点击任意段落进行编辑
       - 点击段落后，该段落进入编辑模式，显示可编辑的文本框
       - 用户可直接在文本框中修改内容
       - 编辑完成后，用户可通过以下方式保存：
         - 点击段落外部区域（失焦）
         - 点击保存按钮（如有）
         - 按下快捷键（如 Ctrl+S 或 Cmd+S）
       - 保存后，段落退出编辑模式，恢复为普通显示状态
       - **段落更新后，自动触发 LLM 分析**：
         - 系统自动调用 LLM 对更新后的段落进行分析
         - 分析内容包括：段落逻辑分析、建议补充内容、建议修改内容
         - 分析结果实时显示在右侧段落分析区
     - **段落可点击**：点击左侧段落后，右侧显示该段落的分析与建议

2. **右侧AI互动与段落逻辑建议区（宽度占比较小）**
   - **段落分析区**：
     - **显示时机**：
       - 用户点击左侧段落时显示
       - 用户编辑完段落并保存后自动显示
   ...
   - **实时协作框**：
     ...
     - 在左侧输出更新后的段落内容
   - **对话区（位于右侧模块最下方）**：
     - 支持用户与CoWrite LLM进行对话
     - 用户可通过对话按用户想法修改对应文章内容
     - 包括对话历史、用户输入框、发送按钮
...
4. **引用标记可视化**
   - 当 LLM 使用资料时，在对应位置插入引用标记（数字标识，如[1]、[2]等）
   - 点击引用标记后，弹窗展示：
     - 资料标题
     - 资料摘要
     - 资料来源 URL
