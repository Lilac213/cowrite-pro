...
export async function rerankMaterialsWithEmbedding(
  ...
  console.log('[rerankMaterialsWithEmbedding] 开始基于 embedding 重排序');
  console.log('[rerankMaterialsWithEmbedding] 材料数量:', materials.length);
  console.log('[rerankMaterialsWithEmbedding] Query:', query);
  console.log('[rerankMaterialsWithEmbedding] 限制内容长度:', maxContentLength, '字符');

  try {
    const { createEmbedding, createBatchEmbeddings, cosineSimilarity } = await import('./embedding.ts');

    const queryEmbeddingResult = await createEmbedding(query, model);
    const queryEmbedding = queryEmbeddingResult.embedding;
    console.log('[rerankMaterialsWithEmbedding] Query embedding 生成成功');

    const textsToEmbed = materials.map(m => 
      `${m.title}\n\n${m.content.substring(0, maxContentLength)}`
    );

    console.log('[rerankMaterialsWithEmbedding] 开始并行生成', textsToEmbed.length, '个 embedding');
    const embeddingResults = await createBatchEmbeddings(textsToEmbed, model, 5);
    console.log('[rerankMaterialsWithEmbedding] 并行 embedding 完成，成功:', embeddingResults.length);
    ...
    const result = scored.map((m, index) => ({
      ...m,
      is_selected: index < topN,
    }));

    console.log('[rerankMaterialsWithEmbedding] 重排序完成，前', topN, '条默认选中');
    console.log('[rerankMaterialsWithEmbedding] Top 1 相似度:', result[0]?.similarity_score);

    return result;
  } catch (error) {
    console.error('[rerankMaterialsWithEmbedding] Embedding 重排序失败，回退到关键词匹配:', error);
